...
(function($, undefined){
 ...
 $(event.target).unbind(endEvent, wrappedCallback)
        ...
        if (duration > 0) {
            ...
            setTimeout(function(){
                if (fired) 
                    return wrappedCallback.call(that)
...
$(function($){
    $("img").each(function(){
        var $this = $(this), w = $this.width(), ww = $this.attr("width"), hh = $this.attr("height"); 
        if (ww && hh) {
            $this.css("height", Math.round(w * hh / ww))
        }
    }).unveil(200, function(){
    ...
    $(document.body).on('click','a[data-type]',function(ev){
        var $this = $(this),type = $this.attr('data-type'),id = $this.attr('data-id'); 
        if(type == 1 || type == 2 || type == 13){
            try{ 
                window.ArtandObject.buyOrder(JSON.stringify({id : id, type : type}));
            }catch(e){ 
                return;
            } 
            return false;
